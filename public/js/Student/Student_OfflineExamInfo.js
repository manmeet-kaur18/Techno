            ...
            for (var x = 0; x < msg.length; x++) {
                if (msg[x].status == "checked" || msg[x].status == "recheck") {
                    ...
                    if (msg[x].status == "checked") {
                        ...
                        var str = "ViewExam('" + msg[x].examID + "','" + msg[x].filename + "','" + msg[x].status + "')";
                    ...
                    dict1[msg[x].examID] = msg[x].filename;
                }
            }
        },
    });
...
var x;
function ViewExam(examID, fileName, status) {
    if (status == "checked") {
...
document.getElementById('Examid').onchange = function () {
    ...
    if (document.getElementById('Examid').value in dict1) {
        filename = dict1[document.getElementById('Examid').value]
    }
    ViewExam(document.getElementById('Examid').value, filename, "done");
}